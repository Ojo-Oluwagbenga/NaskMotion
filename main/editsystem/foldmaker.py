...
def storyfold(imgurls, **kwargs):
    ...
    for key, val in sub_imgdata.items():
        imgdata[key] = pilreadimgonline(imgurls[ic]).convert("RGBA")
        ic += 1
    ...
    for dashline in dashvectors_front:
        draw_dash(dashline[0], dashline[1], 1, parfront)

    for dashline in dashvectors_back:
        draw_dash(dashline[0], dashline[1], 1, parback)

    for line in linevectors_front:
        draw_line(line[0], line[1], 2, parfront)
...
def test(rot):
    ...
    # img = topil(createblank(20, 200))
    # img.save('out.png')
    # read image as RGB and add alpha (transparency)
    # im = Image.open("name.jpg").convert("RGBA")

    # interpolateFold([('E',im)])
    # frag = fragment(im, [
    #     [(0, 0), (100,0), (100,100), (0, 100)], 
    #     [
    #         [
    #             [(0, 0), (100,0), (100,100), (0, 100)], # ['--The sub cut---'],
    #             rot, #degree of rot
    #             (50, 50), #coord of fixture of child
    #             (200, 200), #coord of fixture on parent
    #         ],
    #     ]
    # ])[0]

    # frag[0].save('out3.png')
    

    # im2  = frag[0]
    # cd = frag[1]
    # im.paste(im2, box=(cd[0], cd[1]))

    # im2 = cuttoshape(im2, [(0,0), (100,0), (150,211)])

        # cc = tocv2(im2)
        # rotim = imutils.rotate_bound(cc, rot, center=(0, 0))
        # imrot = topil(rotim)

        # imrot = im2.rotate(rot, expand = True)

        # cc.save('out.png')
        # cv2.cvtColor(cc, cv2.CV_BGR2RGB)
        # cv2.imwrite('out.png', cc)

        # im.paste(im2.rotate(rot, expand = 1), box=(-50, 100))

        # w, h = (im2.size)
        # imrot = im2.rotate(rot, expand = True, resample=Image.BICUBIC)
        # im.paste(imrot, (0, 100), imrot)
        # im.paste(imrot, box=(100, 100))

        # im.save('out.png')

        # imrot = im2.rotate(0, expand = 1, )
        # im.paste(im2,(100, 100))
        # im.save('out2.png')
        

        # im = im.rotate(rot, expand = 1, center=(0,0))
        

        
        # im = cuttoshape(im, [(0,0), (100,0), (150,211)])


        # draw_dash((0,0), (150,211), 15, 5, im)
        # draw_line((100,0), (150,211), im)

        # trimage = cuttoshape(im, [(10,10), (100,10), (150,60), (90,80), (120,90), (10,100)])
        # im = im.resize((30, 30), resample=Image.BICUBIC )
        # w, h = im.size
        # im = pilcrop(im, (0,w/2), (0,h/2))

        # im = im.resize((400, 700))
        # cc = numpy.array(im)[:, :, ::-1].copy() 
        # cc = cv2.cvtColor(numpy.array(im), cv2.COLOR_RGBA2BGRA)
        ...
        # draw_dash((0,0), (100,50), 5, 5, newIm)
        # # draw = ImageDraw.Draw(newIm)
        # # draw.line((0,0, 10,200), fill='green', width= 4)

        # newIm.save("out.png") 

        # image = (crop(cv2.imread('out.png', -1), (0,0), 100, 100))

        # i2 = cv2.line(image, (0, 0), (10, 10), (0, 255, 0), thickness=14)
